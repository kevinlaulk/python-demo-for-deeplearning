...
class fileread(object):
    def __init__(self):
        self.datapath = '../data/'
        self.scriptpath = ''
        self.Annotations = 'F:/研究生/手动特征提取程序/VOCdevkit/VOC2007/Annotations/'
        self.JPEGImages = 'F:/研究生/手动特征提取程序/VOCdevkit/VOC2007/JPEGImages/'
        self.target = 'Brace_sleeve_screw'
    ...
    def JPEGread1(self):
        if not os.path.exists(self.JPEGImages):
            ...
            self.JPEGlistdir = []
            self.JPEGlistname = []
            for root, dirs, files in os.walk(self.JPEGImages):
                for i in files:
                    self.JPEGlistname.append(i.split('.')[0])
                    self.JPEGlistdir.append(root + '/' + i)
        # print(self.JPEGlistdir)
        # print(self.JPEGlistname)
